<template>
  <v-row class="px-3 mb-12 pb-12">
    ...
    <v-dialog v-model="destroy.destroy" max-width="512px">
      <v-card>
        <v-card-title class="headline">Delete Invoice of {{destroy.item.invoiced_at}}</v-card-title>
        <v-card-text>Are you sure that you want to permanently delete Invoice of {{destroy.item.invoiced_at}} ?</v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "INVOICE DATE", value: "invoiced_at" },
  ...
  methods: {
    ...
    editItem(item) {
      ...
      axios.get(`/api/sales/${item.id}`).then(res => {
        ...
        axios
          ...
          .then(res => {
            if (res.status == 201) {
              ...
              this.snackbar.message = `201 (${res.data.data.invoiced_at} Updated Successfully)`;
              this.$emit("update", {
      ...
      if (this.destroy.destroy && this.destroy.index > -1) {
        axios
          ...
          .then(res => {
            ...
            if (res.status == 200) {
              this.snackbar.color = "success";
              this.snackbar.message = `200 (${res.data.data.invoiced_at} Deleted Successfully)`;
